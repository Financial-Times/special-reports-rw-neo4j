...
func main() {
	app := cli.App("special-reports-rw-neo4j", "A RESTful API for managing Special Reports in neo4j")
	...
	app.Action = func() {
		...
		endpoints := map[string]baseftrwapp.Service{
			"special-reports": specialReportsDriver,
		}
		...
		baseftrwapp.RunServer(endpoints,
			v1a.Handler("ft-special-reports_rw_neo4j ServiceModule", "Writes 'special reports' to Neo4j, usually as part of a bulk upload done on a schedule", checks...),
			*port, "special-reports-rw-neo4j", "local")
	}
...
func makeCheck(service baseftrwapp.Service, cr neoutils.CypherRunner) v1a.Check {
	return v1a.Check{
		BusinessImpact:   "Cannot read/write special reports via this writer",
		Name:             "Check connectivity to Neo4j - neoUrl is a parameter in hieradata for this service",
		PanicGuide:       "TODO - write panic guide",
		Severity:         1,
		TechnicalSummary: fmt.Sprintf("Cannot connect to Neo4j instance %s with at least one special report loaded in it", cr),
